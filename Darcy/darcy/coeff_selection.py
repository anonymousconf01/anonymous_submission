...
def apply_WT(u, x_vec, y_vec, jx, jy, kx, ky, chunk_size=128):
    ...
    # print("all device:", u.device, x_vec.device, y_vec.device, jx.device, jy.device, kx.device, ky.device)
...
def apply_Gram(v, dx, dy, x_vec, y_vec, jx, jy, kx, ky):
    return apply_WT(apply_W(v, x_vec, y_vec, jx, jy, kx, ky, chunk_size=128), x_vec, y_vec, jx, jy, kx, ky, chunk_size=128) * dx * dy + lambda_reg * v
...
def conjugate_gradient(b, dx, dy, x_vec, y_vec, jx, jy, kx, ky, max_iter=50, tol=1e-2):
    ...
    # 🔑 Compute norm of b once
    b_norm = torch.norm(b) + 1e-12  # avoid divide by zero
