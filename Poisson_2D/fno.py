...
ntest = 10
# Data Loader
train_data = np.load("data/poisson_data_1.npz", allow_pickle=True)
...
# save the model
# torch.save(model.state_dict(), 'model/fno_poisson.pth')
# load the model
model.load_state_dict(torch.load('model/fno_poisson.pth'))
# Testing
predictions = []
for i in range(ntest):
    ...
    print(f"xtest: {x_test1[0, 0, 0].item():.4f} L2 relative error: {l2_error.item():.4f}")
