...
class DataConfig:
    ...
    def generate_training_points(self):
        ...
        t_ic = self.t_lower * torch.ones(self.n_initial).to(self.device)
        ...
        tfno = torch.linspace(self.t_lower, self.t_upper, self.n_fno)

        x_grid, t_grid = torch.meshgrid(xfno, tfno)
        x_fno = x_grid.reshape(-1)
        t_fno = t_grid.reshape(-1)
