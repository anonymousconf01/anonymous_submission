...
class AWPINN(nn.Module):
    ...
    def forward(self, x, y):
        ...
        dx_wavelets = (self.wx.t()) * (x_transformed**2 - 1) * x_exp
        dy_wavelets = (self.wy.t()) * (y_transformed**2 - 1) * y_exp
        ...
        scale = torch.sqrt(torch.clamp(self.wx.t() * self.wy.t(), min=1e-12))
        # scale = torch.sqrt(self.wx.t() * self.wy.t())
        wavelets = scale * x_wavelets * y_wavelets
        output = (wavelets @ self.output_weight.t()).squeeze() + self.output_bias
        
        # Derivatives
        du_dx = scale * (dx_wavelets * y_wavelets) @ self.output_weight.t()
        du_dy = scale * (x_wavelets * dy_wavelets) @ self.output_weight.t()
        d2u_dx2 = scale * (d2x_wavelets * y_wavelets) @ self.output_weight.t()
        d2u_dy2 = scale * (x_wavelets * d2y_wavelets) @ self.output_weight.t()

        
        return output, du_dx.squeeze(), du_dy.squeeze(), d2u_dx2.squeeze(), d2u_dy2.squeeze()